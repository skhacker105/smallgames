...
export class LudoComponent extends BaseComponent {
  ...
  rollDice(): void {
    ...
    setTimeout(() => {
      ...
      this.rolling = false;
      this.processDiceRoll(this.totalDiceRoll);
      this.saveGameState();
    }, 1000);
  }
  ...
  processDiceRoll(diceRoll: number): void {
    ...
    if (this.playableCoins.size === 0) {
      // Add code to reverse previous move if it was a six and the coin moved. It now needs to be moved back
  ...
  playCoin(coin: ILudoCoin): void {
    ...
    if (this.totalDiceRoll === 6 && coin.position === 0) {
      this.getCoinOutOfBase(coin);
      this.playableCoins.clear();
    }
    else if (coin.position !== 0) {
      this.moveCoin(coin, this.totalDiceRoll)
        .then(() => 6 !== this.totalDiceRoll ? this.moveToNextPlayer() : null);
    }
